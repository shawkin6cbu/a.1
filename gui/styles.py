...
QCheckBox::indicator {
    width: 12px;
    height: 12px;
    border: 1px solid #8ab4f8; /* Blue outline */
    border-radius: 3px;        /* Keep radius, or adjust to 2px if 3px is too much for 12px */
    background-color: #3c4043; /* Background of the inner part when unchecked */
}
QCheckBox::indicator:hover {
    border-color: #a1c9fa; /* Lighter blue for hover on border */
}
QCheckBox::indicator:checked {
    background-color: #8ab4f8;
    border-color: #5f6368;    /* Ensure border stays blue */
    image: url(x_mark.svg);   /* Use X mark SVG */
}
QCheckBox::indicator:checked:hover {
    background-color: #a1c9fa; /* Lighter blue for hover on background */
    border-color: #5f6368;    /* Keep border consistent on hover */
    /* image: url(x_mark.svg); Default image should persist unless overridden */
}
QCheckBox::indicator:disabled {
    border-color: #5f6368;     /* Darker, less prominent border for disabled */
    background-color: #2a2b2d;
    image: none; /* Remove image for disabled state if it shouldn't show X */
}
QCheckBox::indicator:disabled:checked {
    background-color: #5f6368; /* Darker blue/grey for disabled checked state */
    border-color: #5f6368;
    image: url(x_mark.svg); /* Or a greyed-out version of x_mark, or none */
                            /* For now, keep the X but it will be on a muted background */
}
...
QRadioButton::indicator {
    width: 12px;
    height: 12px;
    border: 2px solid #5f6368; /* Blue outline */
    border-radius: 8px;        /* Half of width/height for perfect circle */
    background-color: #3c4043; /* Background of the inner part when unchecked */
}
QRadioButton::indicator:hover {
    border-color: #a1c9fa; /* Slightly lighter blue for hover on border */
}
QRadioButton::indicator:checked {
    background-color: #8ab4f8; /* Inner dot color when checked */
    border-color: #5f6368;
}
/* Styling for the small inner dot of the checked radio button, if needed explicitly.
   Often, the main indicator's background-color when :checked is enough.
   If QSS supports sub-controls like ::dot, it would be here.
   However, for simple radio buttons, ::indicator:checked's background usually IS the dot.
*/
QRadioButton::indicator:checked:hover {
    background-color: #a1c9fa; /* Slightly lighter blue for hover on dot */
    border-color: #a1c9fa;    /* Keep border consistent with dot on hover */
}
QRadioButton::indicator:disabled {
    border-color: #3c4043;
    background-color: #2a2b2d;
}
QRadioButton::indicator:disabled:checked { /* Ensure disabled checked state is also styled */
    background-color: #5f6368; /* Darker blue/grey for disabled checked state */
    border-color: #3c4043;
}
