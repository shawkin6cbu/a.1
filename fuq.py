...
# --- START: PDF Text Extraction Function ---
...
# --- END: PDF Text Extraction Function ---
...
# --- START: Contract Parsing Function with ALL LATEST REFINEMENTS ---
def parse_any_legacy_contract_text(original_text_from_pdf):
    ...
    return data
# --- END: Contract Parsing Function ---

# --- Main Execution Block ---
if __name__ == "__main__":
    pdf_file_path = r"C:\Scripts\Contracta.pdf"
    pdf_directory = os.path.dirname(pdf_file_path)
    pdf_basename = os.path.splitext(os.path.basename(pdf_file_path))[0]
    output_labels_file = os.path.join(pdf_directory, f"{pdf_basename}_extracted_labels_filtered.txt")
    raw_text_output_file = os.path.join(pdf_directory, f"{pdf_basename}_RAW_extracted_text_DEBUG.txt")
    cleaned_text_output_file = os.path.join(pdf_directory, "extracted", f"{pdf_basename}_GLOBALLY_CLEANED_text.txt")

    try:
        raw_extracted_text_from_pdf = extract_text_from_pdf(pdf_file_path) 
        with open(raw_text_output_file, "w", encoding="utf-8") as rt_file:
            rt_file.write(raw_extracted_text_from_pdf)
        print(f"Raw extracted text saved to: {raw_text_output_file}")

        extracted_dir = os.path.join(pdf_directory, "extracted")
        if not os.path.exists(extracted_dir):
            os.makedirs(extracted_dir)
            print(f"Created directory: {extracted_dir}")
        
        globally_cleaned_text_to_save = preprocess_text_globally(raw_extracted_text_from_pdf)
        if globally_cleaned_text_to_save:
            with open(cleaned_text_output_file, "w", encoding="utf-8") as ct_file:
                ct_file.write(globally_cleaned_text_to_save)
            print(f"Globally cleaned text saved to: {cleaned_text_output_file}")
        
        print("Parsing contract data (with strategic preprocessing)...")
        extracted_data = parse_any_legacy_contract_text(raw_extracted_text_from_pdf)
        print("Parsing complete.")

        print(f"Saving extracted labels (only if value found) to: {output_labels_file}")
        with open(output_labels_file, "w", encoding="utf-8") as outfile:
            for key, value in extracted_data.items():
                if value is not None:
                    outfile.write(f"{key}={value}\n")
        print(f"Labels saved successfully to {output_labels_file}")

    except FileNotFoundError:
        print(f"ERROR: The PDF file was not found at {pdf_file_path}")
    except ImportError:
        print("ERROR: The 'pdfminer.six' library is not installed. Please install it using 'pip install pdfminer.six'")
